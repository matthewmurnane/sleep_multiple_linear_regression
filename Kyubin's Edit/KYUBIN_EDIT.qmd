...
#GVIF high for occupation, stress, bmi category
#GVIF^ high for quality of sleep


#stress
sleep$stress_level_num <- as.numeric(as.factor(sleep$stress_level))
...
# R^2 .9542 suggest we remove it

#occupation
sleep$occupation_num <- as.numeric(as.factor(sleep$occupation))

aux_model_occupation <- lm(occupation_num ~ gender + age + quality_of_sleep + 
                           physical_activity_level + bmi_category + heart_rate + 
                           daily_steps + sleep_disorder + MAP + stress_level, 
                           data = sleep)

summary(aux_model_occupation)
# R^2 around .7232 keep occupation

# BMI
sleep$bmi_category_num <- as.numeric(as.factor(sleep$bmi_category))
...
#quality
sleep$quality_of_sleep_num <- as.numeric(as.factor(sleep$quality_of_sleep))
aux_model_quality <- lm(quality_of_sleep_num ~ gender + age + physical_activity_level +
                         ...
                         sleep_disorder + MAP + stress_level,
                         data = sleep)
